...
icon = Image.open("youtube1.png")
st.set_page_config(page_title="Youtube Harvesting and Warehousing",
                   page_icon=icon,
                   layout="wide",
                   initial_sidebar_state="expanded",
                   menu_items={'About': """# This app is created by *Chemosh!*"""})
...
with st.sidebar:
    selected = option_menu(None, ["Home","Extract and Transform","View"], 
                           ...
                           styles={"nav-link": {"font-size": "30px", "text-align": "left", "margin": "0px", 
                                                "--hover-color": "#C80101"},
                                   "icon": {"font-size": "30px"},
...
if selected == "Home":
    # Title Image
    st.image("youtube2.jpg")
    col1,col2 = st.columns(2,gap= 'medium')
    col1.markdown("## :blue[Domain] : Social Media")
    col1.markdown("## :blue[Technologies used] : Python, Streamlit, Youtube Data API, MongoDB, MySql")
...
api_key = "AIzaSyBLNHLUn8t6s9WmNmQzjAOcUQd5pVOFV4U"
...
if selected == "Extract and Transform":
    ...
    def create_table_channels():
        query1 = "create table if not exists channels (Channel_id varchar(255), Channel_name varchar(255), Playlist_id varchar(255), Subscribers int, Views int, Total_videos int, Description text, Country varchar(255));"
        mycursor.execute(query1)
        mydb.commit()

    def create_table_videos():
        query2 = "create table if not exists videos (Channel_name varchar(255), Channel_id varchar(255), Video_id varchar(255), Title varchar(255), Tags varchar(255), Thumbnail varchar(255), Description text, Published_date datetime, Duration int, Views int, Likes int, Comments int, Favorite_count int, Definition varchar(255), Caption_status varchar(255));"
        mycursor.execute(query2)
        mydb.commit()

    def create_table_comments():
        query3 = "create table if not exists comments (Comment_id varchar(255), Video_id varchar(255), Comment_text text, Comment_author varchar(255), Comment_posted_date datetime, Like_count int, Reply_count int);"
        mycursor.execute(query3)
        mydb.commit()

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  
    # INSERT TABLES IN MYSQL
    def insert_into_channels():
        collections1 = db.channel_details
        query4 = """INSERT INTO channels VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"""
        for i in collections1.find({"Channel_name" : user_inp},{'_id' : 0}):
            mycursor.execute(query4,tuple(i.values()))
            mydb.commit()
                                                                                                                    
    def insert_into_videos():
        collections2 = db.video_details
        query5 = """INSERT INTO videos VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
        for i in collections2.find({"Channel_name" : user_inp},{'_id' : 0}):
            mycursor.execute(query5,tuple(i.values()))
            mydb.commit()
        
    def insert_into_comments():
        collections3 = db.video_details
        collections4 = db.comments_details
        query6 = """INSERT INTO comments VALUES(%s,%s,%s,%s,%s,%s,%s)"""
        for vid in collections3.find({"Channel_name" : user_inp},{'_id' : 0}):
            for i in collections4.find({'Video_id': vid['Video_id']},{'_id' : 0}):
                mycursor.execute(query6,tuple(i.values()))
                mydb.commit()
